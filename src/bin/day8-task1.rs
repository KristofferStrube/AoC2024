    ...
    for group in antennas.values() {
        for first in group {
            for second in group {
                ...
                let first_antinode = (
                    if positive_x { first.0 - (second.0 - first.0) } else { first.0 + (first.0 - second.0) },
                    if positive_y { first.1 - (second.1 - first.1) } else { first.1 + (first.1 - second.1) }
                );
                if get_char(&grid, first_antinode.0, first_antinode.1) != '#' {
                    antinodes.insert(first_antinode);
                }
                let second_antinode = (
                    if positive_x { second.0 + (second.0 - first.0) } else { second.0 - (first.0 - second.0) },
                    if positive_y { second.1 + (second.1 - first.1) } else { second.1 - (first.1 - second.1) }
                );
